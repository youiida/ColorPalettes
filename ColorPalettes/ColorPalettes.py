...
class ColorPalettes:
    # All color palettes are stored in this dictionary
    palettes = {
        ...
        'romantic_roses': ['#f9f6f7', '#a53d59', '#d5a8aa', '#582341'],
        'pastel_hues': ['#adafba', '#f77a9f', '#e8e4ec', '#747884'],
        'vibrant_mix': ['#fe3781', '#ff77ad', '#ecf660', '#747884'],
        'ocean_breeze': ['#2c2e3a', '#00ada7', '#3a4357', '#9ca1cf'],
        'fresh_greens': ['#b7dec3', '#dfa0bd', '#005397', '#ddec8d'],
        'earthy_neutrals': ['#8ad0bb', '#7e412a', '#fffeef', '#e66684'],
        'classic_shades': ['#a83d49', '#9a9c97', '#ece7e9', '#4c4951'],
        'alt_black': {
            '#1d2131': 'charcoal',
            '#30384e': 'slate',
            '#323942': 'graphite',
    ...
    def get_palette(palette_name, num_colors=None):
        ...
        palette = ColorPalettes.palettes.get(palette_name)
        if palette is None:
    ...
    def display_group_palettes(group_name):
        # First, define the main groups
        group_palettes = {
            'Tetracolor': [
                name for name, palette in ColorPalettes.palettes.items()
        ...
        # Then, define the 'Additional Palettes' group
        group_palettes['Additional Palettes'] = [
            name for name in ColorPalettes.palettes.keys()
            if name not in [name for group in ['Tetracolor', 'Alt Black', 'Bicolor', 'Tricolor'] for name in group_palettes[group]]
        ]
        ...
        if num_palettes == 1:
            fig, axes = plt.subplots(
                num_palettes, 1, figsize=(20, 2 * num_palettes))
            axes = [axes]
        else:
            fig, axes = plt.subplots(
        ...
        for ax, palette_name in zip(axes, palettes):
            palette = ColorPalettes.palettes[palette_name]
            if isinstance(palette, list):
                for i, color in enumerate(palette):
                    ax.add_patch(plt.Rectangle((i, 0), 1, 1, color=color))
            elif isinstance(palette, dict):
